...
const handleDeviceMessage = async (topic, message) => {
  ...
  const deviceType = hasDigit
    ? topicArray.slice(0, topicArray.length - 1).join("-")
    : topic;
  const deviceName = hasDigit
    ? topicArray[topicArray.length - 2] + topicArray[topicArray.length - 1]
    : topicArray[topicArray.length - 1];
  ...
  switch (deviceType) {
    ...
    case "w-humi":
      ...
      break;
    case "w-human":
      updateQuery = { value: data === "1" ? true : false };
      break;
    case "w-led":
      updateQuery = { value: data, status: data === "#000000" ? false : true };
      break;
    case "w-fan":
      updateQuery = {
  ...
  } else if (deviceType === "w-human") {
    deviceModel = HumanDetection;
